    ...
    fun create(): RhythmChart {
        val events = buildList {
            ...
            repeat(8) { bar ->
                ...
                add(RhythmEvent(time, if (bar % 2 == 0) DrumLane.HI_HAT else DrumLane.RIDE, 94))
                add(RhythmEvent(time + 500, DrumLane.SNARE, 120))
                add(RhythmEvent(time + 500, DrumLane.HI_HAT, 90))
                add(RhythmEvent(time + 1_000, DrumLane.KICK, 112))
                add(RhythmEvent(time + 1_000, DrumLane.HI_HAT, 92))
                add(RhythmEvent(time + 1_500, DrumLane.SNARE, 122))
                add(RhythmEvent(time + 1_500, DrumLane.HI_HAT, 92))
                time += 2_000
            }
